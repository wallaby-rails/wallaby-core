...
describe Wallaby::ResourcesController, type: :controller do
  ...
  describe '.models && .models_to_exclude && .all_models' do
    it 'returns models' do
      ...
      expect(described_class.models_to_exclude.to_a).to include(ActiveRecord::InternalMetadata) if version?('>= 6.0') && version?('< 7.1')
    ...
    context 'when subclass' do
      ...
      it 'returns models' do
        ...
        expect(application_controller.models_to_exclude.to_a).to include(ActiveRecord::SchemaMigration) if version?('< 7.1')
