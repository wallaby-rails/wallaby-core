...
describe Wallaby::Guesser do
  describe '.class_for' do
    ...
    it 'returns the associated class' do
      ...
      expect(described_class.class_for(Core::Special::SupermenController.name, suffix: 'Decorator')).to eq Special::SupermanDecorator
    end

    context 'when not found' do
      it 'returns nil' do
        expect(described_class.class_for(Core::Special::SupermenController.name)).to be_nil
      end
    end
    ...
    context 'when there are many classes meet the rules' do
      it 'returns the one matches the most' do
        ...
        expect(described_class.class_for(Core::Special::SupermenController.name)).to eq Special::Superman
      end
    end
  end
end
