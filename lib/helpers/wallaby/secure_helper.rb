...
module Wallaby
  ...
  module SecureHelper
    ...
    def logout_path(user: wallaby_user, app: main_app, method_name: controller_configuration.try(:logout_path))
      method_name ||=
        if defined? ::Devise
    ...
    def logout_method(user: wallaby_user, http_method: controller_configuration.try(:logout_method))
      http_method ||
        if defined? ::Devise
          scope = ::Devise::Mapping.find_scope! user
